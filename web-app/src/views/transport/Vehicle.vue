        ...
        <a-col :span="8" v-for="(item, index) in this.data" :key="index" class="item">
          <a-card hoverable>
            <template slot="actions" class="ant-card-actions">
              ...
              <span @click="viewhistory(item.id)">使用记录</span>
            </template>
            <a-card-meta
...
<!--    使用记录model框-->
    <a-modal v-model:visible="visible2" title="使用记录" @ok="handleOk" width="600" >
      <a-table :dataSource="distributionHistory" :columns="columns" >
      </a-table>
    </a-modal>
    ...
    methods: {
      ...
      viewhistory(id){
        FindAllDistribution().then((res) => {
          if (res.status) {
            //筛选配送记录为当前车辆的记录
            this.distributionHistory = res.data.filter(item => item.vid == id)

            if (this.distributionHistory.length==0){
              this.$message.warn("暂无历史记录")
            }else {
              this.visible2=true
            }
          }
