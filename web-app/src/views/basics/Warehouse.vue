        ...
        <a-col :span="8" v-for="(item, index) in this.data" :key="index" class="item">
          <a-card hoverable>
            <template slot="actions" class="ant-card-actions">
              ...
              <span @click="deleteWarehouse(item.id)" type="link"><a-icon type="delete"/> 删除仓库</span>
            </template>
            <a-card-meta
                :title="item.name"
...
<script>
import {FindAllWarehouse, SaveWarehouse,DeleteWarehouseById} from "@/api/warehouse";
...
export default {
  ...
  methods: {
    ...
    ,
    deleteWarehouse(id) {
      DeleteWarehouseById(id).then((res) => {
